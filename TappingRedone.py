...
#             B = (i+1)*self.fs + seconds*self.fs
             shift1 = np.min([np.min(self.gyro1xT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]),np.min(self.gyro1yT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]),np.min(self.gyro1zT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs])])
             shift2 = np.min([np.min(self.gyro2xT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]),np.min(self.gyro2yT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]),np.min(self.gyro2zT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs])])
              
             cropgx1, cropgy1, cropgz1 = self.gyro1xT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]-shift1,self.gyro1yT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]-shift1,self.gyro1zT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]-shift1
             cropgx2, cropgy2, cropgz2 = self.gyro2xT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]-shift2,self.gyro2yT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]-shift2,self.gyro2zT[(i+1)*self.fs:(i+1)*self.fs + seconds*self.fs]-shift2
             
             denom1 = np.max([np.max(cropgx1), np.max(cropgy1), np.max(cropgz1)])
             denom2 = np.max([np.max(cropgx2), np.max(cropgy2), np.max(cropgz2)])
             cropgx1, cropgy1, cropgz1 = cropgx1/denom1, cropgy1/denom1, cropgz1/denom1
             cropgx2, cropgy2, cropgz2 = cropgx2/denom2, cropgy2/denom2, cropgz2/denom2 
...
def plotConfMat(cm,cmPerc):
    ...
    return
#%%
cm, cmPerc = confuseMat(XtestRight,YtestRight,mA)
