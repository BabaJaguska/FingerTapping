...
def create_configurations(configuration_type):
    configurations = None
    if configuration_type == 'random_attr':
        configurations = create_configurations_random_attr(
            Parameters.number_of_configurations, Parameters.attribute_range_values)
    elif configuration_type == 'random_one_attr':
        configurations = create_configurations_random_one_attr(
            Parameters.number_of_configurations, Parameters.attribute_range_values)
    elif configuration_type == 'one_attr':
        configurations = create_configurations_one_attr(
...
def create_configurations_random_one_attr(cnt, attributes_values=(), add_default=1):
    ...
    while len(result) < cnt:
        ...
        index = np.random.randint(0, len(attributes_values))
        attribute = attributes_values[index]
        attribute_name = attribute[0]
        # def_val = attribute[1]
        min_val = attribute[2]
        max_val = attribute[3]
